...
**Recommended workflow:** install and run with **uv** and **[Agent Starter Pack](https://google.github.io/adk-docs/)** (see [Quick start](#quick-start-agent-starter-pack--primary) below).
...
## Quick start (Agent Starter Pack — primary)

This sample ships with **[Agent Starter Pack](https://google.github.io/adk-docs/)** (`agent-starter-pack` 0.32.x in `pyproject.toml`) and is easiest to run with **[uv](https://docs.astral.sh/uv/)**. The `[tool.agent-starter-pack]` section sets an `example_question` for templating.
...
### Google authentication (Vertex / ASP)
...
`llm_auditor/__init__.py` loads `.env`, then uses **`google.auth.default()`** and **`os.environ.setdefault("GOOGLE_CLOUD_PROJECT", …)`** when Vertex is enabled so ADC can supply the project in Agent Starter Pack–style deployments. It skips ADC when `GOOGLE_GENAI_USE_VERTEXAI` is false (Gemini API key path).
